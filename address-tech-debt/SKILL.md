...
This skill runs **after** every issue in a plan is merged and **before** the plan folder is deleted. The goal is to **fix** the tech debt left behind — not to archive it. The plan resolver lives under `<SKILL_DIR>/scripts/`. `<SKILL_DIR>` is the directory containing this `SKILL.md`.
